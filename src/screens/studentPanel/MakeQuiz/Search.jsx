...
function Search(props) {
  ...
  const back = React.useCallback(() => {
    ...
    if (mode === 'lesson' || (mode === 'finalize' && lastMode === 'branch')) {
      ...
      const tmp = props.flags.filter(itr => {
        return itr.section === 'subject' && itr.lessonId === filterId;
      });
      if (tmp.length > 0) {
        const tmpLesson = props.flags.find(itr => {
  ...
  const addToSelected = shouldReturn => {
    ...
    if (mid > 0) {
      const obj2 = JSON.parse(JSON.stringify(selectingItem));
      obj2.level = 'mid';
      obj2.count = mid;
      tmp.push(obj2);
    }
    if (hard > 0) {
      const obj3 = JSON.parse(JSON.stringify(selectingItem));
      obj3.level = 'hard';
      obj3.count = hard;
      tmp.push(obj3);
    }
